...
def Calculate_rho_Cp(P_s,T_s, alpha):
    P = P_s*1e-03 # P in GPa; T in K
    T = T_s
    ...
    result = result/(molar_mass*1e-03)
    
    # print("cp is " + str(result))
...
def general(Ps, Ts, MassWi, rb, rhocore):
    ...
    '''new approimation'''
    alpha_VII = 11.58e-5
    rho_VII, Cp_VII = Calculate_rho_Cp(P_s,T_s,alpha_VII)    
    ...
    while (massDiff > mass_thrshd):
        ...
        # print("MWi"+str(Mass_W_i))
        # print("MWL"+str(Mass_WL))
        # print("MD"+str(massDiff)+"\n")
        ...
        for k in range(g_it) if (massDiff==100 or massDiff<mass_thrshd) else range(1):
            ...
            if P_s > 2200:
                # out.rho = rho_VII
                out.alpha = alpha_VII
                out.rho, out.Cp = Calculate_rho_Cp(P_s,T_s,out.alpha)
                # print("first VII used for Cp to be "+str(out.Cp))
                phase_s[0] = 7
                print("!!!")
            else:
            ...
            for i in range(z.size-1):  # Integration with depth 
                ...
                if P[i+1] > 2200:
                    ...
                    # print("T is "+str(T[i+1])+" and Tm is "+str(Tm))
                    if T[i+1] < Tm:
                        '''implemetation'''
                        out.rho, out.Cp = Calculate_rho_Cp(P[i+1],T[i+1],alpha[i])
                        '''expansibity changes according to change in volume/density'''
                        out.alpha = (out.rho - rho[i])/(out.rho*np.abs(T[i+1]-T[i]))
                        # print("number "+str(i)+" VII used for Cp to be "+str(out.Cp))
                        phase[i+1] = 7
                        print("!!!")
                    else:
                        phase[i+1] = 0
